  ...
  <div class="min-h-screen flex">
    ...
    <div class="flex flex-1 items-center justify-center bg-slate-50 px-6">
      <div class="w-full max-w-md">
        <h2 class="text-2xl font-bold text-slate-900 text-center mb-6">{{ $t('auth.register') }}</h2>
        ...
        <div class="text-center text-sm text-slate-500">
          {{ $t('auth.hasAccount') }}
          <router-link to="/login" class="text-indigo-600 hover:text-indigo-700 font-medium">{{ $t('auth.login') }}</router-link>
        </div>
      </div>
    </div>
  </div>
