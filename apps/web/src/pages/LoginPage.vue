<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-50">
    <el-card class="w-full max-w-md">
      <template #header>
        <h2 class="text-xl font-semibold text-center">{{ $t('auth.login') }}</h2>
      </template>
      <el-form ref="formRef" :model="form" :rules="rules" @submit.prevent="handleSubmit">
        ...
        <el-form-item prop="password">
          <el-input v-model="form.password" :placeholder="$t('auth.password')" type="password" show-password />
        </el-form-item>
        <el-form-item>
      ...
      <div class="text-center text-sm">
        {{ $t('auth.noAccount') }}
        <router-link to="/register" class="text-blue-500">{{ $t('auth.register') }}</router-link>
      </div>
    </el-card>
  </div>
</template>
